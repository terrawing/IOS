...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(updateUI:) name:@"GlovesList_updateUI_fetch_completed" object:nil];
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(gwGetAll:) name:@"Model_glovesGetAll_fetch_completed" object:nil];
    _arr = self.model.glovesGetAll;
...
- (void)gwAddNewItem:(NSNotification *)notification
{
    ...
    [self.model gwGetAllRefresh:@"gloves" andPropertyName:@"glovesGetAll"];
    return;
    /*
...
- (void)AddGlovesController:(id)controller didEditItem:(id)item
{
    ...
    if (item)
    {
        [self.model addNewEquipmentItem:(NSDictionary *)item andEquipmentName:@"gloves"];
    }
...
{
    if ([segue.identifier isEqualToString:@"toAddGloves"])
    {
        // Configure the next view + controller
        UINavigationController *nav = (UINavigationController *)segue.destinationViewController;
        AddGloves *nextVC = (AddGloves *)nav.topViewController;
        
        nextVC.model = self.model;
        nextVC.delegate = self;
    }
    
    if([segue.identifier isEqualToString:@"toGlovesDetailNetworkView"])
    {
        ...
        // Configure the next view + controller
        GlovesDetailNetworkView *nextVC = (GlovesDetailNetworkView *)segue.destinationViewController;
        
        nextVC.title = [NSString stringWithFormat:@"%@ %@", o[@"Brand"], o[@"Model"]];
        nextVC.model = self.model;
        nextVC.glovesDictionary = o;
    }
...
{
    [self performSegueWithIdentifier:@"toGlovesDetailNetworkView" sender:indexPath];
}
